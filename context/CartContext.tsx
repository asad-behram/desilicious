...
import { CartItem, MenuItem } from '../utils/interface';
  ...
  switch (action.type) {
    case 'ADD_ITEM':
      ...
      if (existingItem) {
        ...
        return {
          items: newItems,
          total: newItems.reduce((sum, item) => sum + (item.item.price * item.quantity), 0)
        };
      }
    case 'REMOVE_ITEM':
      ...
      return {
        items: filteredItems,
        total: filteredItems.reduce((sum, item) => sum + (item.item.price * item.quantity), 0)
      };
